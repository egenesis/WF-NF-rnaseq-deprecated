...
if (params.fasta) {
  if (hasExtension(params.fasta, 'gz')) {
    ...
    Channel.fromPath(params.fasta, checkIfExists: true)
        .ifEmpty { exit 1, "Genome fasta file not found: ${params.fasta}" }
        .into { ch_fasta_for_star_index; ch_fasta_for_hisat_index; ch_gfasta_for_salmon_index }
  }
...
if (compressedReference) {
  ...
  if (params.fasta && (alignment_no_indices || pseudoalignment_no_indices)) {
    process gunzip_genome_fasta {
        ...
        output:
        file "${gz.baseName}" into ch_fasta_for_star_index, ch_fasta_for_hisat_index, ch_fasta_for_salmon_transcripts, ch_gfasta_for_salmon_index
...
if (params.pseudo_aligner == 'salmon' && !params.salmon_index) {
    ...
    process makeSalmonIndex {
        ...
        file gfasta from ch_gfasta_for_salmon_index
        ...
        """
        grep "^>" $gfasta | cut -d " " -f 1 > decoys.txt
        sed -i.bak -e 's/>//g' decoys.txt
        cat $fasta $gfasta > gentrome.fa
        salmon index --threads $task.cpus -d decoys.txt  -t gentrome.fa $gencode -i salmon_index
        """
    }
}
